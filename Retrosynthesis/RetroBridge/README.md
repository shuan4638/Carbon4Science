...
For simple programmatic access, use the `Inference.py` module:
...
# Basic usage - predict reactants for a product SMILES
results = run("CC(=O)OC1=CC=CC=C1C(=O)O")  # Aspirin
print(results)
# ['CC(=O)Cl.O=C(O)c1ccccc1O', ...]  # Acetyl chloride + Salicylic acid

# With custom parameters
results = run(
    smiles="CN1C=NC2=C1C(=O)N(C(=O)N2C)C",  # Caffeine
    n_samples=10,       # Number of predictions to generate
    n_steps=500,        # Diffusion steps (more = better quality)
    seed=42,            # Random seed for reproducibility
)

# Get RDKit molecule objects along with SMILES
smiles_list, rdkit_mols = run(smiles, return_rdkit=True)
```

### Command Line

```bash
python Inference.py "CC(=O)OC1=CC=CC=C1C(=O)O"
```
...
|-----------|---------|-------------|
| `smiles` | required | Product molecule SMILES string |
| `n_samples` | 10 | Number of reactant predictions to generate |
| `n_steps` | 500 | Number of diffusion steps (higher = better quality, slower) |
| `checkpoint_path` | `models/retrobridge.ckpt` | Path to model checkpoint |
| `device` | auto | Device: `'cuda:0'`, `'cpu'`, or `None` for auto-detect |
| `seed` | 42 | Random seed for reproducibility |
| `return_rdkit` | False | If True, also return RDKit molecule objects |
...
# Reset the cached model (useful for loading a different checkpoint)
reset_model()
```
