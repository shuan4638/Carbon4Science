...
def xyz2mol(atoms, coordinates, charge=0, allow_charged_fragments=True,
    ...
    try:
        ...
        flag = 'fail'
    return new_mols, flag


def xyz2mol_run(xyz_path, output_mol_path=None, no_charged_fragments=False, no_graph=False,
        ignore_chiral=True, use_huckel=False, use_charge=None):
    # read xyz file
    filename = xyz_path
    ...
    if use_charge is not None:
        charge = int(use_charge)

    # Get the molobjs
    mols, flag = xyz2mol(atoms, xyz_coordinates,
                         charge=charge,
                         use_graph=quick,
                         allow_charged_fragments=charged_fragments,
                         embed_chiral=embed_chiral,
                         use_huckel=use_huckel)
    if flag == 'success' and output_mol_path is not None:
        molblock = Chem.MolToMolBlock(mols[0])
        smiles = Chem.MolToSmiles(mols[0])
        with open(output_mol_path + '_' + smiles + '.sdf', 'w') as f:
            f.write(molblock)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="latent diffusion model (LDM) ")
    parser.add_argument("--xyz_path", type=str, default='/mnt/e/tangui/latent_diffusion/outputs/QM9_3D_ddpm/xyz')
    parser.add_argument("--sdf_path", type=str, default='./outputs/QM9_3D_ddpm/sdf')
    parser.add_argument("--lmdb_path", type=str, default='./data/qm9')
    parser.add_argument("--mode", type=str, default='sdf', help="save lmdb or sdf")
    args = parser.parse_args()

    xyz_file_names = os.listdir(args.xyz_path)

    if args.mode == 'lmdb':
        from multiprocessing import Pool
        txn_write, env_new = write_lmdb(args.lmdb_path, 'mol_train')
        with Pool(processes=120) as pool:
            xyz_file_names = [os.path.join(args.xyz_path, file) for file in xyz_file_names]
            iter = pool.imap(xyz2mol_run, xyz_file_names)
            pbar = tqdm(enumerate(iter), total=len(xyz_file_names))
            for i, (mols, flag) in pbar:
                if flag == 'success' and args.mode == 'lmdb':
                    info = {}
                    info['mol'] = mols[0]
                    txn_write.put(f'{i}'.encode("ascii"), pickle.dumps(info, protocol=-1))
        txn_write.commit()
        env_new.close()

    else:
        os.makedirs(args.sdf_path, exist_ok=True)
        pbar = tqdm(enumerate(xyz_file_names), total=len(xyz_file_names))
        for i, xyz_file in pbar:
            xyz_file_path = os.path.join(args.xyz_path, xyz_file)
            output_file_path = os.path.join(args.sdf_path, xyz_file.split('.')[0])
            mol, flag = xyz2mol_run(xyz_file_path, output_mol_path=output_file_path)
