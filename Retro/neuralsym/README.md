...
## Quick Inference
For the simplest usage, use the `run()` function from `Inference.py`:
```python
...
# Multiple SMILES
results = run(["CCO", "CCCO"])

# Custom top-k predictions (default is 10)
results = run("CCO", topk=5)
```

The function returns a dictionary mapping each input SMILES to a list of predictions:
```python
{
    "COC(=O)c1cccc2[nH]c(NCC3CCNCC3)nc12": [
        {
...
- `precursors`: List of predicted precursor SMILES strings
- `score`: Model probability score
- `template`: The reaction template used to generate the precursors
