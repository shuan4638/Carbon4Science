...
For simple inference without setting up the full pipeline, use the `Inference.py` module:
...
# Run on single SMILES
results = model.run("CCO")

# Run on multiple SMILES
...
# Run predictions
results = run("CCO")
results = run(["CCO", "CC(=O)O"], n_beams=5)
```
...
python Inference.py \
    --model_path path/to/model.ckpt \
    --vocab_path bart_vocab_downstream.json \
    --task backward_prediction \
...
### Output format
The `run()` function returns a list of dictionaries:
```python
[
    {
        "input": "CCO",
        "predictions": ["C=O", "CC=O", ...],  # predicted SMILES
        "log_likelihoods": [-1.23, -2.45, ...]  # scores for each prediction
    }
]
```
