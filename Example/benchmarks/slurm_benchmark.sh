...
#SBATCH -o Example/benchmarks/logs/%x.o%j  # output file (%x=job name, %j=jobID)
...
#   sbatch --job-name=RetroBridge Example/benchmarks/slurm_benchmark.sh RetroBridge
...
MODEL=${1:?Usage: sbatch Example/benchmarks/slurm_benchmark.sh MODEL_NAME [extra args...]}
shift
EXTRA_ARGS="$@"
...
OUTPUT="Example/results/outputs/${MODEL,,}_${N}.json"
PREDICTIONS="Example/results/outputs/${MODEL,,}_${N}_predictions.json"
...
if [ "$MODEL" = "Chemformer" ]; then
    export PYTHONPATH="$(pwd)/Example/Chemformer:$PYTHONPATH"
fi
...
PYTHONUNBUFFERED=1 python Example/benchmarks/run_benchmark.py \
    --model "$MODEL" \
    --track_carbon \
    --output "$OUTPUT" \
